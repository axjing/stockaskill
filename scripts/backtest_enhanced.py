...
ETF_TOTAL = sum(e["target"] for e in ETF_CORE)
...
def _trend_filter(kline, ma_short=50, ma_long=200) -> bool:
...
def run_backtest():
    ...
    def _get_price(code: str, date: str) -> float:
        ...
        if code in index_kline:
            return index_kline.get(date, 0)
        return 0

    def _get_index_kline(up_to_date):
        return [
            {"close": index_kline[d]}
